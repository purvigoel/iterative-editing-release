...
class InfillWrapper:
    def __init__(self, model, diffusion, data_template, dist_util, max_frames=meoConstant.max_frames):
        ...
        self.time_warp = TimeWarp(max_frames)
        self.time_warp_traj = TimeWarp(max_frames)
    ...
    def prepare_sample(self, input_motions, keyframe, trajectory_sample, window, hi_window, frames, i, is_root_edit):
        ...
        inpainting_mask = torch.zeros(original.shape).to(original.device) + 0.1
        ...
        if not is_root_edit:
            ...
            for frame in frames:
                ...
                for fr in range(lo, frame):
                    ...
                    inpainting_mask[..., fr] = 0.1 + (1.0 - 0.1) * a 

                for fr in range(frame, hi):
                    a = (fr - frame) / (hi - frame)
                    og_motion[:, :, :, fr] = og_motion[:, :, :, fr] + (1 - a) * (delta)
                    inpainting_mask[..., fr] = 0.1 + (1.0 - 0.1) * ( 1 - a) 
        ...
        return keyframed_version, interpolated_version, keyframe_mask, inpainting_mask
    ...
    def forward(self, original_motion, keyframe, trajectory_sample, lo_window, hi_window, frames, i = 0, is_root_edit=False ):
        ...
        keyframed_version, interpolated_version, keyframe_mask, inpainting_mask = self.prepare_sample(original_motion, keyframe, trajectory_sample, lo_window, hi_window, frames, i, is_root_edit)
        ...
        inpainting_mask = inpainting_mask.to(self.dist_util.dev())
        ...
        if not is_root_edit:
            ...
            model_kwargs["y"]["inpainting_mask"] = clonemask #inpainting_mask #clonemask #keyframe_mask.clone() #torch.ones(original.shape).to(original.device).to(torch.bool)
        ...
        def model_fn(x, t, **model_kwargs):
            return self.diffusion.p_mean_variance(self.model, x, t,
                clip_denoised=False, model_kwargs=model_kwargs)['pred_xstart']  # this gives substantially better results than below!!
        ...
        return sample, keyframed_version #interpolated_version

    def forward2(self, original_motion, keyframe, trajectory_sample, lo_window, hi_window, frames, i = 0, is_root_edit=False ):
        iterator = iter(self.data_template)
        input_motions, model_kwargs = next(iterator)
        ...
        model_kwargs["y"]["keyframe"] = interpolated_version.squeeze(2).permute(0, 2, 1)
        noise = torch.randn(keyframed_version.shape).to(keyframed_version.device)
        ...
        if not is_root_edit:
            ...
            clonemask = keyframe_mask.clone()
            clonemask = torch.where(keyframe_mask, 1.0, 0.1)
            model_kwargs["y"]["inpainting_mask"] = clonemask
        ...
        original = original_motion.clone()
        model_kwargs["y"]["noise_mask"] = keyframe_mask.clone()
        model_kwargs["y"]["original"] = interpolated_version.clone()
