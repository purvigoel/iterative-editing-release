...
from pose_utils import rigid_transform_3D, apply_rigid_transform
...
def compute_g_mpjpe(g_pred_j3ds, g_target_j3ds):
    ...
    def g_mpjpe_per_sequence(g_pred, g_target):
        ...
        print(1000 * np.sqrt(((g_pred_transformed - g_target)**2).sum(-1)))
        return np.sqrt(((g_pred_transformed - g_target)**2).sum(-1)).mean(), g_pred_transformed

    N = len(g_pred_j3ds)
    errors = np.zeros((N,))
    transfs = np.zeros(g_pred_j3ds.shape)
    for i in range(N):
        g_pred = g_pred_j3ds[i]
        g_target = g_target_j3ds[i]
        error, transf = g_mpjpe_per_sequence(g_pred, g_target)
        errors[i] = error
        transfs[i] = transf
    return errors, transfs
