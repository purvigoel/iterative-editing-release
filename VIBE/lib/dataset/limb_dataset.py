...
class LimbKeyframeDataset(Dataset):
    ...
    def get_single_item(self, index):
        ...
        # data: 236, 1, N
        keyframes = data.clone()
        keyframe_ind = np.random.randint(1, 59)

        lo_window = max(0, keyframe_ind - 12)
        hi_window = min(keyframe_ind + 12, 60)

        # second pass: given a trajectory of spine/pelvis with some full keyframes, infill the rest
        keyframes[ 6 : 24*6, :, lo_window:keyframe_ind] = 0
        keyframes[ 6 : 24*6, :, keyframe_ind + 1:hi_window] = 0

        keyframes[25 * 6 :, :, keyframe_ind+1:hi_window] = 0
        keyframes[25 * 6 :, :, lo_window:keyframe_ind] = 0
        #keyframes[150:154, :, keyframe_ind+1:hi_window] = -1
        #keyframes[150:154, :, lo_window:keyframe_ind] = -1

        mask = torch.ones(keyframes.shape).to(torch.bool)
        mask[ 6 : 24*6, :, lo_window:keyframe_ind] = 0
        mask[ 6 : 24*6, :, keyframe_ind + 1:hi_window] = 0

        mask[25 * 6 :, :, keyframe_ind+1:hi_window] = 0
        mask[25 * 6 :, :, lo_window:keyframe_ind] = 0
        

        keyframes = keyframes.squeeze().permute(1,0)
        if self.motion_cond:
            ret = dict(
                inp=data.float(),
                action_text='',
                vid_name=torch.zeros(60, 236),
            )
        elif self.keyframe_cond:
            ret = dict(
                inp=data.float(),
                action_text='',
                vid_name = torch.zeros(60,236),
