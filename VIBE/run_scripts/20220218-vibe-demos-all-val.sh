...
fname=$(ls /scratch/groups/syyeung/hmr_datasets/h36m_val/${sn}/Videos/  | sed -n ${vid_id}p)
fname=$(printf %q "$fname") # Escapae whitespaces

cmd="python demo.py \
	--vid_file=/scratch/groups/syyeung/hmr_datasets/h36m_val/${sn}/Videos/${fname} \
	--output_folder=/scratch/users/wangkua1/results/vibe/${sn}/${fname}
"
...
sbatch <<< \
"#!/bin/bash
#SBATCH --job-name=vibe-demos6
#SBATCH --output=slurm_logs/vibe-demos6-%j-out.txt
#SBATCH --error=slurm_logs/vibe-demos6-%j-err.txt
#SBATCH --mem=48gb
#SBATCH -c 1
#SBATCH --gres=gpu:1
#SBATCH -p syyeung
