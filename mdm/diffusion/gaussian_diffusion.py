#s code is based on https://github.com/openai/guided-diffusion
"""
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        self, model, x, t, inpaint_image=None , clip_denoised=True, denoised_fn=None, model_kwargs=None
    ):
        """
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        if "trick" in model_kwargs['y'].keys():
            x = model_kwargs['y']['inpainted_motion']
        ...
        if 'inpainting_mask' in model_kwargs['y'].keys() and 'inpainted_motion' in model_kwargs['y'].keys():
            ...
            if "trick" in model_kwargs['y'].keys():
                if t[0] < 1:
                    model_output = ratio *( (model_output * ~inpainting_mask) + (inpainted_motion * inpainting_mask) ) + (1 - ratio) * model_output
                else:
                    model_output = x
            else:
                if t[0] > 0:
                    inpaint_image = self.q_sample(inpainted_motion.clone(), t[0] - 1)
                else:
                    inpaint_image = inpainted_motion.clone()
                #model_output = ratio *( (model_output * ~inpainting_mask) + (inpainted_motion * inpainting_mask) ) + (1 - ratio) * model_output
    ...
    def ik_solution(self, model, sample_start, inpaint, limb, magnitude, framenums, xloc=-0.1, yloc=0.25, zloc=1.0, contact=[]):
        device = sample_start.device
        N = sample_start.shape[0]
        s_s = []
        i_s = []
        if framenums:
            for f in range(len(framenums)):
                frame_n = framenums[f]
                s_s.append(sample_start[f, ..., frame_n].unsqueeze(-1))
                i_s.append(inpaint[f, ..., frame_n].unsqueeze(-1))
            sample_start = torch.stack(s_s)
            inpaint = torch.stack(i_s)
        else:
            pass
        joint_id = None
        if limb == "left_foot":
            joint_id = [7]
        elif limb== "right_foot" or limb == "right_ankle":
            joint_id = [8]
        elif limb == "left_hand":
            joint_id=  [20]
            ...
            for j in range(len(limb)):
                if limb[j]== "left_foot":
                    joint_id.append(7)
                elif limb[j]== "right_foot":
                    joint_id.append(8)
                elif limb[j] == "left_hand":
                    joint_id.append(20)
                elif limb[j] == "right_hand":
                    joint_id.append(21)
        
        inpaint_normalized = inpaint.clone()

        inpaint_joint_locations = model.forward_kinematics(inpaint_normalized, None)
        inpaint_joint_locations = inpaint_joint_locations["kp_45_joints"][:, :24].reshape(N, 24,3) +  torch.tensor([0, 0.2256, 0]).unsqueeze(0).unsqueeze(0).to(device)
        inpaint_joint_locations = inpaint_joint_locations.to(device)
        inpaint_joint_locations_c = inpaint_joint_locations.clone()
        root_orient = inpaint[:, 0:6, ...].clone().squeeze(-1).squeeze(-1)
        
        root_orient_aa = geometry.rotation_6d_to_aa(root_orient)
        root_orient_mat = batch_rodrigues(root_orient_aa).view(-1, 3, 3)

        if len(contact) == 0:
           for j in range(inpaint_joint_locations.shape[0]):
              tmp_magnitude = magnitude[j].clone()
              tmp_magnitude[1] = 0.0 
              if( magnitude[j, 0] != 0.0 or magnitude[j, 2] != 0.0):
                   ...
                   diff = diff * oldnorm
                   print(j, len(joint_id))
                   prev = inpaint_joint_locations[j, joint_id[j], :].clone()
                   inpaint_joint_locations[j, joint_id[j], :] += diff
                   new = inpaint_joint_locations[j, joint_id[j], :].clone()

           if not isinstance(limb, list):
               inpaint_joint_locations[:, joint_id, 1] += dir_vector[1]
           else:
               for j in range( inpaint_joint_locations.shape[0] ):
                   inpaint_joint_locations[j, joint_id[j], 1] += magnitude[j, 1]
        elif len(contact) == 2:
            inpaint_joint_locations[:, pose_mask.pose_dict[contact[0]], :] = inpaint_joint_locations[:, pose_mask.pose_dict[contact[1]], :].clone()  + torch.tensor(magnitude).unsqueeze(0).unsqueeze(0).to(inpaint_joint_locations.device)
        else:
            
            inpaint_joint_locations[:, pose_mask.pose_dict[contact[0]], :] = inpaint_joint_locations[:, pose_mask.pose_dict[contact[0]], :].clone()  + torch.tensor(magnitude).unsqueeze(0).unsqueeze(0).to(inpaint_joint_locations.device)

        T = 1
        sample_clone = sample_start.clone()
        with th.enable_grad(): 
            angles = Variable(sample_start, requires_grad = True)
            opt = optim.Adam([angles], lr=1e-1) # 1e-2
            
            target_joint_locations = model.forward_kinematics(sample_start, None)
            target_joint_dic = target_joint_locations
            target_joint_locations = target_joint_locations["kp_45_joints"][:, :24].reshape(N, 24,3)+  +  torch.tensor([0, 0.2256, 0]).unsqueeze(0).unsqueeze(0).to(angles.device) #torch.tensor(magnitude).unsqueeze(0).unsqueeze(0).to(angles.device)
 
            target_joint_locations = target_joint_locations.to(device)
            target_joint_locations_c = target_joint_locations.clone()

            if not isinstance(limb, list):
                target_joint_locations[:, joint_id, 1] = inpaint_joint_locations[:, joint_id, 1].clone()  
                target_joint_locations[:, joint_id, 0] = inpaint_joint_locations[:, joint_id, 0].clone()
                target_joint_locations[:, joint_id, 2] = inpaint_joint_locations[:, joint_id, 2].clone()
            else:
                for j in range( inpaint_joint_locations.shape[0]):
                    target_joint_locations[j, joint_id[j], :] = inpaint_joint_locations[j, joint_id[j], :]
            
            coords = [0, 1, 2] 
            itrs = 100
            target_joint_locations = target_joint_locations + target_joint_dic["pred_trans"].reshape(N, T, 3)
           
            for itr in range(itrs):
                pred = model.forward_kinematics(angles, None)
                pred_joints = pred["kp_45_joints"][:, :24].reshape(N, 24,3) +  torch.tensor([0, 0.2256, 0]).unsqueeze(0).unsqueeze(0).to(angles.device) + pred["pred_trans"].reshape(N, T, 3) + pred["pred_trans"].reshape(N,1, 3) 
                lossa = torch.norm(target_joint_locations[:, 7, coords] - pred_joints[:, 7, coords])
                lossb = torch.norm(target_joint_locations[:, 20, coords] - pred_joints[:, 20, coords])
                lossc = torch.norm(target_joint_locations[:, 21, coords] - pred_joints[:, 21, coords])
                lossd = torch.norm(target_joint_locations[:, 8, coords] - pred_joints[:, 8, coords])
                losse = torch.norm(angles[:, pose_mask.spine_angles_nr, :, :] - sample_clone[:, pose_mask.spine_angles_nr, :, :])
                lossf = torch.norm(angles[:, :6, :, :] - sample_clone[:, :6, :, :])
                loss = losse * 0.0 + lossa + lossb + lossc + lossd + lossf * 0.0
                if len(contact) > 0 and contact[1] in pose_mask.pose_dict:
                    loss += torch.norm(target_joint_locations[:, pose_mask.pose_dict[contact[1]], coords] - pred_joints[:, pose_mask.pose_dict[contact[1]], coords])
                opt.zero_grad()
                loss.backward()
                opt.step()
            print(loss)
    ...
    def p_sample(
        ...
        if ("upsample_infill" not in model_kwargs["y"].keys() and 'constraint' not in model_kwargs['y'].keys() and 'stitch' not in model_kwargs['y'].keys() and 'ik' not in model_kwargs["y"].keys()) or ('stitch' in model_kwargs['y'].keys() and t[0] < 20) or ("ik" in model_kwargs['y'].keys() and t[0] < 3) or ('constraint' in model_kwargs['y'].keys() and t[0] < 20) or ("upsample_infill" in model_kwargs["y"].keys() and t[0] < 1001):
            ...
            if const_t_noise:
                ...
                if "noise_mask" in model_kwargs["y"]:
                    noise = torch.where(model_kwargs["y"]["noise_mask"], model_kwargs["y"]["original"], noise)
            ...
            inpainting_mask, inpainted_motion = model_kwargs['y']['inpainting_mask'], model_kwargs['y']['inpainted_motion']
            inpaint_motion_noise = self.q_sample(inpainted_motion.clone(), t - 1) if (t[0] > 0) else inpainted_motion
            if "keyframe_mask" in model_kwargs["y"]:
                inpaint_motion_noise = torch.where(model_kwargs["y"]["keyframe_mask"], inpainted_motion, inpaint_motion_noise)
            sample = inpaint_motion_noise 
            out = {"mean": inpainted_motion , "pred_xstart": inpainted_motion }

        # do IK on the mean
        # (noisy sample) * blend_ratio + (blend_ratio * IK solution) 
        if 'ik' in model_kwargs['y'].keys():
            if 'ratio' in model_kwargs['y'].keys():
                thresh = model_kwargs['y']['ratio']
            inpainting_mask, inpainted_motion = model_kwargs['y']['inpainting_mask'], model_kwargs['y']['inpainted_motion']
            if t[0] > 1:
                inpaint_motion_noise = self.q_sample(inpainted_motion.clone(), t - 1) if (t[0] > 0) else inpainted_motion
                sample = inpaint_motion_noise
            else:
                ratio = model_kwargs['y']['blend']
                ratio = 1.0
                edit_limb = model_kwargs['y']['limb']
                magnitude = model_kwargs['y']['mag']

                contact = []
                if "contact" in  model_kwargs["y"].keys():
                    contact = model_kwargs["y"]["contact"]
                if "framenum" in model_kwargs["y"].keys():
                    ik_sample = self.ik_solution(grad_model, out["mean"], model_kwargs['y']['inpainted_motion'], edit_limb, magnitude, model_kwargs["y"]["framenum"], contact=contact)                
                else:
                    ik_sample = self.ik_solution(grad_model, out["mean"], model_kwargs['y']['inpainted_motion'], edit_limb, magnitude, None, contact=contact)

                ik_sample = self.q_sample(ik_sample.clone(), t - 1) if (t[0] > 0) else ik_sample
                if "framenum" in model_kwargs["y"].keys():
                    for s in range(sample.shape[0]):
                        sample[s, ..., model_kwargs["y"]["framenum"][s]] = ik_sample[s, ..., 0]
                else:
                    sample = ik_sample

        elif 'inpainting_mask' in model_kwargs['y'].keys() and 'inpainted_motion' in model_kwargs['y'].keys():
            inpainting_mask, inpainted_motion = model_kwargs['y']['inpainting_mask'], model_kwargs['y']['inpainted_motion']
            inpaint_motion_noise = self.q_sample(inpainted_motion.clone(), t - 1) if (t[0] > 0) else inpainted_motion
            ...
            if t[0] > thresh:
                ...
                if 'blend' in model_kwargs['y'].keys():
                    ratio = model_kwargs['y']['blend'] #0.25
        
            if 'force' in model_kwargs['y'].keys():
                ratio = 1
            if not skip:
                sample = ratio * (sample * (1 - inpainting_mask) + (inpainting_mask * inpaint_motion_noise)) + (1 - ratio) * sample 

        sample_inpaint = None #self.q_sample(inpaint_image_clean, t[0]) 
        return {"sample": sample, "sample_inpaint": sample_inpaint, "pred_xstart": out["pred_xstart"], "mean": out["mean"]}
    ...
    def p_sample_loop_progressive(
        ...
        inpaint_image = None        
        if 'inpainting_mask' in model_kwargs['y'].keys() and 'inpainted_motion' in model_kwargs['y'].keys():
            inpainting_mask, inpaint_image_clean = model_kwargs['y']['inpainting_mask'].clone(), model_kwargs['y']['inpainted_motion'].clone()   
            inpaint_image = inpaint_image_clean
            inpaint_image = model_kwargs['y']['inpainted_motion'].clone()
        ...
        early_stop = False
        early_stop_ind = 900 #T = 1000 is the same as full. 
        ...
        for i in indices:
            ...
            counter += 1
            if early_stop and counter > early_stop_ind:
                break
            if use_mean:
                img = out['mean']
            else:
                img = out["sample"]
                inpaint_image = out["mean"]
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None, foot_vel_threshold=0.01):
        ...
        if not use_regression:
            x_start_clone = x_start.clone()
            keyframe_mask = torch.tensor(model_kwargs["y"]["keyframe_mask"]).to(x_start_clone.device)
            x_t = self.q_sample(x_start, t, noise=noise)
            # save keyframe context
            x_t = torch.where(keyframe_mask, x_start_clone, x_t)
        else:
            x_t = th.zeros(x_start.shape).to(x_start.device)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.lambda_vel_rcxyz > 0.:
                if self.data_rep in ('rot6d','rot6d_fc', 'rot6d_fc_shape'):# and dataset.dataname in ['humanact12', 'uestc', 'amass', 'amass_hml','h36m','3dpw']:
